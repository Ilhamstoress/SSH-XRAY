...
echo -e "${WH}Host: $domen ${NC}" | tee -a /etc/log-create-user.log
echo -e "${WH}[crlf]Connection: Keep-Alive[crlf]User-Agent: [ua][crlf]Upgrade: ws[crlf][crlf]${NC}" | tee -a /etc/log-create-user.log
echo -e "$COLOR1◇━━━━━━━━━━━━━━━━━◇${NC}"  | tee -a /etc/log-create-user.log
echo -e "$COLOR1◇━━━━━━━━━━━━━━━━━━◇${NC}" | tee -a /etc/log-create-user.log
echo -e "$COLOR1 ${NC}     ${WH}• IlhamStore •${NC}       $COLOR1 $NC" | tee -a /etc/log-create-user.log
echo -e "$COLOR1◇━━━━━━━━━━━━━━━━━━◇${NC}" | tee -a /etc/log-create-user.log
...
wget -q -O /usr/bin/proxy3.js "https://raw.githubusercontent.com/Ilhamstoress/SSH-XRAY/main/data/proxy3.js"
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}" 
wget -q -O /usr/bin/ssh-wsenabler "https://raw.githubusercontent.com/Ilhamstoress/SSH-XRAY/main/data/sshws-true.sh" && chmod +x /usr/bin/ssh-wsenabler
...
echo -e "$COLOR1====================${NC}"
echo -e "$COLOR1 ${NC}  ${WH}GET / HTTP/1.1[crlf]Host: [host][crlf]Connection: Keep-Alive[crlf]Upgrade: websocket[crlf][crlf]${NC}"
echo -e "$COLOR1====================${NC}" 
echo -e "$COLOR1=============${WH}BY${NC}${COLOR1}==============${NC}"
